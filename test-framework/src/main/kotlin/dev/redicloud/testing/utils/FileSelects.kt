...
data class ProjectFileSelect(
    ...
    val file: File
        get() {
            ...
            require(project.exists()) { "Project $projectName does not exist" }
            val build = File(project, "build")
            require(build.exists()) { "Project $projectName does not have a build folder" }
            val libs = File(build, "libs")
            require(libs.exists()) { "Project $projectName does not have a libs folder" }
            val files =
